    ...
    for (let node of this.nodes) {
      if (!node.tombstone) {
        if (count === displayIndex) {
          return (
            (node.position +
              this.nodes[this.nodes.indexOf(node) + 1].position) /
            2
          );
        }
        count++;
...
function testCRDT() {
  ...
  console.assert(
    crdt_client.nodes[1].letter === "a",
    "Client insertDisplayIndex failed"
  );
  ...
  console.assert(
    crdt_server.nodes[1].letter === "a",
    "Server insertPosition failed"
  );
  ...
  console.assert(
    crdt_client.nodes[1].tombstone === true,
    "Client deleteDisplayIndex failed"
  );
  ...
  console.assert(
    crdt_server.nodes[1].tombstone === true,
    "Server deletePosition failed"
  );
...
io.on("connection", async (socket) => {
  ...
  socket.on("insert", (node) => {
    ...
    console.log("documentCRDTs", documentCRDTs.keys());
    console.log("documentCRDTs", documentCRDTs.get(docId));
  ...
  socket.on("disconnect", () => {
    ...
    // if this is the last user in the room need to save the crdt to the database
